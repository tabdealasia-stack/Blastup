...
import { Folder, Plus, Search, MoreVertical, Edit2, Trash2, ShieldBan, CheckCircle } from 'lucide-react';
...
export default function CategoriesPage() {
  ...
  const [formData, setFormData] = useState({ name: '', slug: '', description: '', active: true, defaultTemplatePackId: '' });
  ...
  const handleOpenModal = (category: any = null) => {
    if (category) {
      ...
      setFormData({
        ...
        defaultTemplatePackId: category.defaultTemplatePackId || '',
      });
    } else {
      setEditingCategory(null);
      setFormData({ name: '', slug: '', description: '', active: true, defaultTemplatePackId: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        defaultTemplatePackId: formData.defaultTemplatePackId || null,
      };

      if (editingCategory) {
        await tabdealApi.updateCategory(editingCategory._id, payload);
      } else {
        await tabdealApi.createCategory(payload);
      }
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this category? This action cannot be undone.')) {
      try {
        await tabdealApi.deleteCategory(id);
        fetchCategories();
      } catch (err: any) {
  ...
  return (
    <>
      <Header title="Categories" subtitle="Manage client business categories" />
      ...
      <div className="page-content" style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '48px', color: '#64748b' }}>Loading categories...</div>
        ) : error ? (
          ...
          <div style={{ textAlign: 'center', padding: '64px', background: '#f8fafc', borderRadius: '12px', border: '1px dashed #cbd5e1' }}>
            <Folder size={48} style={{ color: '#94a3b8', margin: '0 auto 16px' }} />
            <h3 style={{ margin: '0 0 8px', color: '#334155' }}>No Categories Found</h3>
            <p style={{ margin: 0, color: '#64748b' }}>Create your first business category to get started.</p>
          </div>
        ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                <tr>
                  ...
                  <th style={{ padding: '16px', textAlign: 'right', color: '#64748b', fontWeight: 500, fontSize: '13px' }}>Packs</th>
                ...
                {filteredCategories.map(cat => (
                  <tr key={cat._id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: '16px' }}>
                      <div style={{ fontWeight: 500, color: '#0f172a' }}>{cat.name}</div>
                      <div style={{ fontSize: '13px', color: '#64748b', marginTop: '2px' }}>{cat.slug}</div>
                    </td>
                    <td style={{ padding: '16px' }}>
                      {cat.active ? (
                    ...
                    <td style={{ padding: '16px', textAlign: 'right', color: '#334155' }}>{cat.clientCount}</td>
                    <td style={{ padding: '16px', textAlign: 'right', color: '#334155' }}>{cat.packCount}</td>
                    <td style={{ padding: '16px', textAlign: 'right' }}>
                      <button onClick={() => handleOpenModal(cat)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6366F1', marginRight: '16px' }}>
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(cat._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ef4444' }}>
                        <Trash2 size={16} />
                      </button>
                    </td>
                  </tr>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15, 23, 42, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#fff', padding: '32px', borderRadius: '12px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ margin: '0 0 24px', fontSize: '20px' }}>{editingCategory ? 'Edit Category' : 'Create Category'}</h2>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <input 
                  ...
                  required
                />
                <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#64748b' }}>Used for URLs and matching. (e.g. travel-agency)</p>
              </div>
